...
void LocationNode::PublishImuOdometry(const ros::TimerEvent &event){
    ...
    if (gps_status_ == GpsStatus::VALID) {
		...
		imu_odom.pose.pose.position.x = current_cart_.x;
		imu_odom.pose.pose.position.y = current_cart_.y;
		imu_odom.pose.pose.position.z = current_cart_.z;
    } else {
